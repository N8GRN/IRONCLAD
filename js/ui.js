...
(function (IC) {
  IC.icon = function (name) {
    var paths = {
      ...
      ladder: '<path d="M8 21.2 10.2 2.8"/><path d="M16 21.2 13.8 2.8"/><path d="M9.1 6.6h5.6"/><path d="M8.7 10.6h6.4"/><path d="M8.4 14.6h7"/><path d="M8.1 18.4h7.6"/>',
  ...
  IC.customerQuoteRows = function (job, est, settings) {
    ...
    if (c && c.discountPercent > 0 && c.listTotal != null) {
      var excluded = (c.addonsSubtotal || 0) + (Number(c.deliveryFee) || 0) + (Number(c.equipmentRental) || 0) + (Number(c.miscSubtotal) || 0);
      prices = IC.structurePrices(est.structures, c.listTotal, excluded, c.measuredSquares);
    }
    if (!prices.length) {
    ...
    if (IC.normalizeMisc && IC.MISC_DEFS) {
      var misc = IC.normalizeMisc(est.misc);
      var miscSettings = settings || (IC.state && IC.state.settings) || IC.SETTINGS;
  ...
  IC.jobSheetHtml = function (job, settingsName) {
    ...
    var body = !est
      ...
      : '<p class="muted" style="margin-top:1rem">Materials only. For the lumber yard order.</p>' +
        ...
        "</tbody></table>";
    var miscNotes = [];
    if (est && IC.normalizeMisc && IC.MISC_DEFS) {
    ...
    if (miscNotes.length) {
      body += '<section class="est-comments"><p class="est-k">Install notes</p>' + miscNotes.join("") + "</section>";
    }
  ...
  IC.navySumHtml = function (c, opts) {
    ...
    var materialCost = (Number(c.materialsSubtotal) || 0) + (Number(c.deckingMaterialCost) || 0) + (Number(c.miscMaterialCost) || 0);
    var laborPrice = Number(c.laborSubtotal) || 0;
    var rows = [
      ["Labor Price", laborPrice],
    ];
    if ((Number(c.miscSubtotal) || 0) > 0) rows.push(["Miscellaneous", Number(c.miscSubtotal) || 0]);
    rows.push(["Materials", materialCost]);
    if (opts.actual !== false) {
  ...
  IC.jobCostHtml = function (job) {
    ...
    var materialCost = round2(materials + decking + num(c.miscMaterialCost));
    ...
    var miscLabor = num(c.laborCostMisc);
    var roofLabor = round2(laborCost - accessory - osbLabor - woodLabor - miscLabor);
    ...
    var materialLines = lines.filter(function (l) { return l.kind === "material" || l.kind === "decking" || l.kind === "yard"; });
    var quoteNote = "";
    ...
    var summary = '<header class="est-head"><div class="est-co"><h3>' + IC.esc(settings.legalName || "IRONCLAD Roofing") + '</h3>' +
      ...
      row("Decking", decking) +
      row("Miscellaneous materials", num(c.miscMaterialCost), "Skylight yard price. Step flashing, if any, is in Materials.") +
      row("Crew labor", roofLabor, "Install and tear-off, paid on measured squares") +
      row("Miscellaneous labor", miscLabor, "Skylight, satellite dish, and antenna") +
